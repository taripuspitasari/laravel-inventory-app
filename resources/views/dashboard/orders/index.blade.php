                ...
                <td x-data="{open:false}" class="relative">
                    ...
                    <div x-show="open" x-on:mouseenter="open = true" x-on:mouseleave="open = false" class="absolute z-50 right-1 top-0" x-cloak>
                        <div class="join">
                            ...
                            <div class="btn btn-error join-item bg-white">
                                <form action="/dashboard/orders/{{ $order->id }}/cancel" method="post">
                                @method('put')
                                @csrf
                                <button class="text-wrap" onclick="return confirm('Are you sure you want to cancel this order?')">Cancel</button>
                                </form>
                            </div>
                        </div>
                    </div>
