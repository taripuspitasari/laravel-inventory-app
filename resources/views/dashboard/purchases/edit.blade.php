...
@section('container')
<form class="w-full bg-white p-4 shadow-md rounded-lg" action="/dashboard/transactions" method="post">
    @csrf
    <h3 class="font-bold">INVOICE</h3>
        <div class="space-y-2">
        <div class="grid gap-4 md:grid-cols-2 md:gap-6">
            <input type="hidden" name="transaction_type" value="{{ $type == 'purchase' ? 'in' : 'out' }}">
            <div>
            ...
            <div>
                <label for="partner" class="block mb-2 text-sm font-medium text-gray-500 dark:text-white">{{ $partner }}</label>
                <select id="partner" name="partner_id" class="{{ $errors->has('partner_id') ? 'border-red-500' : 'border-gray-300' }} bg-gray-50 border text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 " required value="{{ old('partner') }}">
            ...
            <div>
                <label for="transaction_status" class="block mb-2 text-sm font-medium text-gray-500 dark:text-white">Status</label>
                <select type="date" name="date" id="date" class="{{ $errors->has('date') ? 'border-red-500' : 'border-gray-300' }} bg-gray-50 border text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required value="{{ old('date') }}">
        ...
        <div>
            <label for="notes" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Notes/Terms</label>
            <textarea id="notes" name="notes" id="notes" rows="3" class="{{ $errors->has('notes') ? 'border-red-500' : 'border-gray-300' }} bg-gray-50 border text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"  placeholder="Enter notes or terms of services" required>{{ old('notes') }}</textarea>
            @error('notes')
            <div class="mt-2 text-sm text-red-600">{{ $message }}</div>
            @enderror
        </div>
