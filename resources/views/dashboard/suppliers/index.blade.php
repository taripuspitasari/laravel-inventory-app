...
<section class="">
    <div class="flex flex-col md:flex-row gap-2 justify-between">
        <div class="w-full md:w-96">
            <form action="/dashboard/suppliers" method="get" class="flex items-center">
                ...
                <div class="relative w-full">
                    ...
                    <input type="search" name="search" value="{{ request('search') }}" id="search" class="block w-full p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Search suppliers" autocomplete="off">
                </div>
            </form>
        </div>
        <div class="flex flex-col md:flex-row gap-2 relative">
            <a href="/dashboard/suppliers/create" class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                ...
                <table class="w-full text-sm text-left text-gray-600 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"><a href="/dashboard/suppliers/{{ $supplier->id }}">{{ $supplier->name }}</a></th>
                            <td class="px-4 py-3 whitespace-nowrap">{{ Str::limit($supplier->address, 70) }}</td>
                            ...
                            <td class="px-4 py-3 flex items-center justify-center" x-data="{open:false}">
                                ...
                                <div x-show="open" x-on:mouseenter="open = true" x-on:mouseleave="open = false" class="absolute z-10 right-5 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600" x-cloak>
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                                        <li>
                                            <a href="/dashboard/suppliers/{{ $supplier->id }}" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Show</a>
                                        </li>
                                        <li>
                                            <a href="/dashboard/suppliers/{{ $supplier->id }}/edit" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                        </li>
                                    </ul>
                                    <div class="py-1">
                                        <form action="/dashboard/suppliers/{{ $supplier->id }}" method="post" class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">
                                        @method('delete')
                                        @csrf
                        ...
                        @empty
                        <tr class="border-b dark:border-gray-700">
                            <td colspan="7" class="px-4 py-3 text-center">No suppliers available</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
