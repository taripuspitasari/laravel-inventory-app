...
<section>
    <div class="flex flex-col gap-2 md:flex-row justify-between">
        <div class="w-full md:w-96">
            <form class="flex items-center" action="/dashboard/suppliers" method="get">
                ...
                <div class="relative w-full">
                    ...
                    <input type="search" name="search" id="search" class="block w-full p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-primary-500 focus:border-primary-500" placeholder="Search suppliers" value="{{ request('search') }}">
                </div>
            </form>
        </div>
        <a href="/dashboard/suppliers/create" class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-xs px-4 py-2" >
    ...
    <div class="overflow-x-auto rounded-md border border-base-content/5 bg-base-100 my-4">
        <table class="table">
            ...
            <tr>
                ...
                <td class="whitespace-nowrap"><a href="/dashboard/suppliers/{{ $supplier->id }}">{{ $supplier->name }}</a></td>
                <td class="whitespace-nowrap">{{ Str::limit($supplier->address, 70) }}</td>
                ...
                <td x-data="{open:false}" class="relative">
                    ...
                    <div x-show="open" x-on:mouseenter="open = true" x-on:mouseleave="open = false" class="absolute z-50 right-1 top-0" x-cloak>
                        <div class="join">
                            <button class="btn join-item bg-white"><a href="/dashboard/suppliers/{{ $supplier->id }}">Show</a></button>
                            <button class="btn join-item bg-white"><a href="/dashboard/suppliers/{{ $supplier->id }}/edit">Edit</a></button>
