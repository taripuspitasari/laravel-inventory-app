...
<div class="p-6 space-y-4 mx-auto md:w-1/2 md:space-y-6 sm:p-8">
    ...
    <form class="space-y-4 md:space-y-6" action="/register" method="post">
        ...
        <div>
            ...
            <input type="text" name="username" id="username" class="{{ $errors->has('username') ? 'border-red-500' : 'border-gray-300' }} bg-gray-50 border text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Username" required value="{{ old('username') }}">
            @error('username')
            <div class="mt-2 text-sm text-red-600">{{ $message }}</div>
            @enderror
        </div>
